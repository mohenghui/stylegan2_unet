...
class Generator(nn.Module):
    ...
    def forward(
        ...
        i = 2
        # print(self.convs)
        for conv1, conv2,conv3, noise1, noise2, noise3,to_rgb in zip(
            self.convs[::3], self.convs[1::3],self.convs[2::3], noise[2::3], noise[3::3], noise[4::3],self.to_rgbs
        ):
            out = conv1(out, latent[:, i], noise=noise1)
            out = conv2(out, latent[:, i + 1], noise=noise2)
            out = conv3(out, latent[:, i + 2], noise=noise3)
